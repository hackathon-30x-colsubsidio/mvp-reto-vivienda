...
export const CATALOGO_RECURSOS = {
  ...
  // ⚠️ NO está mapeado por el mapeador (index.ts) a propósito: ayuda con la
  // cuota INICIAL (el enganche), y ningún factor del motor modela esa barrera.
  // Colgarla del gate del 40% sería deshonesto —ese gate mide la cuota mensual
  // sobre el ingreso, derivada del precio, no del ahorro para el enganche—, el
  // mismo error por el que se sacó compra de cartera del gate. Se conserva en el
  // catálogo como aliado disponible; entra al mapeo el día que exista un factor
  // de capacidad de enganche.
  ahorro_para_mi_casa: {
