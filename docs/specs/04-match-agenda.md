...
> ✅ **Defecto del paso 1, hallado y CERRADO el 2026-07-25** ([discusión de workflow](../agents/discusion-workflow-2026-07-25.md) §2.1 → [ticket 023](../tasks/023-puente-capacidad-antes-del-proyecto.md)). "Cayó en nutrición" se decidía **contra un solo proyecto** —el de entrada, o el más barato si no había— así que un lead que eligió uno caro salía con **cero proyectos** aunque el catálogo tuviera varios dentro de su techo. Medido entonces: ingreso $4.000.000 + ARAUCARIA → `nutricion`, 0 proyectos, con **13 de los 18 cabiéndole**. Era el mismo patrón que ya se había corregido en el cupo (D3): el filtro correcto castigaba al lead equivocado.
>
> El arreglo vive en [`lib/curar.ts`](../../lib/curar.ts) (`referenciaParaCalificar`), **no aquí**: el matcher siempre hizo bien su trabajo — lo que estaba mal era contra qué proyecto se calificaba antes de llamarlo. Hoy, si el proyecto de entrada no cabe pero alguno del catálogo sí, se recalifica contra el más económico que le cabe **y la ficha lo dice con su cuota y su % del ingreso**. Si no cabe ninguno, sigue siendo nutrición: que es lo que nutrición debía significar siempre.
