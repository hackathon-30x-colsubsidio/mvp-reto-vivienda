...
- **Si vas a tocar el conversador con un agente, dale estas cuatro primero.** Son las trampas que costaron tiempo de verdad al cablear la rama 5, y ninguna se deduce leyendo el archivo que estás editando:
  1. **`interpretarTexto` sigue SÍNCRONO y PURO.** [`guion-demo.ts:89`](lib/fixtures/guion-demo.ts) lo llama de forma síncrona y de ahí salen el seed y las fixtures. Si lo vuelves `async` para cablear el intérprete de IA, **revientas el seed**. La capa de IA vive afuera.
  2. **La red de [`lib/conversacion/escenarios/`](lib/conversacion/escenarios/) congela el comportamiento ACTUAL, no el deseado.** Si un test de ahí falla, el refactor cambió comportamiento — no es que el test esté mal. **No lo "arregles".**
  3. **El JSX no se toca** (líneas ~598-889 de [`ChatWhatsApp.tsx`](components/chat/ChatWhatsApp.tsx)), los archivos generados no se editan a mano, y **el copy que lee el lead no se mejora de paso**: es 🔴 consultar.
  4. **`{ id: null }` de `/api/banco` y `{ valor: null }` de `/api/interpretar` son respuestas NORMALES, no errores.** Significan "ninguna vale la pena" y "no entendí". Tratarlas como fallo **apaga la capa entera**.
...
- **⚠️ `\b` no sirve en español: normaliza con `sinTildes` antes de cualquier regex sobre texto del lead.** JavaScript define `\b` sobre `[A-Za-z0-9_]`, así que **las vocales con tilde y la `ñ` no cuentan como letra**. Muerde en las dos direcciones y las dos ya costaron un bug real: `/\bno s[ée]\b/` **nunca** casa con "no sé" (entre `é` y el espacio no hay frontera, son dos no-palabras), y `/peques?\b/` **sí** casa dentro de "pequeño" (entre `e` y `ñ` sí la hay). Tres de los seis bugs de interpretación del 2026-07-26 salieron de aquí. La regla del repo: `sinTildes` de [`lib/conversacion/interpretacion/texto.ts`](lib/conversacion/interpretacion/texto.ts) primero, o un lookahead explícito que incluya `ñ` y las tildes; nunca `\b` pegado a un carácter acentuado.
